    ...
    <div className="flex-1 min-w-0 bg-slate-50/50 p-8 overflow-y-auto">
      ...
      <div className="bg-white rounded-2xl p-8 border border-[#e2e8f0] shadow-sm mb-6 relative overflow-hidden">
        <div className="flex justify-between items-start relative z-10">
          <div className="flex gap-6">
            <div
              className={`w-24 h-24 rounded-2xl ${customer.avatarColor} flex items-center justify-center text-white text-3xl font-bold shadow-lg shadow-green-200`}
            >
              {customer.initials}
            </div>
            <div>
            ...
            <div className="relative w-16 h-16 flex items-center justify-center">
              <svg className="transform -rotate-90 w-16 h-16 absolute inset-0">
                ...
                <circle
                  ...
                  strokeDasharray="176"
                  strokeDashoffset={
                    176 - (176 * customer.stats.engagementScore) / 100
                  }
                  strokeLinecap="round"
              ...
              <span className="text-lg font-bold text-[#0f172a]">
                {customer.stats.engagementScore}
              </span>
            </div>
            <span className="text-xs font-medium text-[#64748b] mt-1">
              Score
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-8 mt-8 py-6 border-t border-[#e2e8f0]">
          <div>
            ...
            <p className="text-xl font-bold text-[#0f172a]">
              SZL {customer.stats.lifetimeValue.toLocaleString()}
            </p>
          </div>
          <div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          {(customer.history || []).map((purchase: any, i: number) => (
            <div
              key={i}
              className="bg-white border border-[#e2e8f0] rounded-xl p-6 hover:shadow-sm transition-shadow"
            >
