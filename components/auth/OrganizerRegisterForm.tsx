...
import { registerOrganizer } from "@/app/actions/auth";
...
export default function OrganizerRegisterForm() {
  const [state, formAction] = useActionState(
    async (prevState: any, formData: FormData) => {
      ...
      const result = await registerOrganizer(prevState, formData);
      if (result.success) {
        toast.success("Organization created! Please verify your email.");
  ...
  // Password strength indicators (simplified) - could be extracted to hook
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="mt-6 text-center text-sm">
        ...
        >
          <ArrowLeftIcon /> Back to Organizer Login
        </Link>
      </div>
    </form>
  );
