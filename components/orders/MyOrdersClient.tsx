  ...
  const handleDownloadPDF = async (order: Order) => {
    ...
    try {
      ...
      const blob = await pdf(
        <TicketDocument order={order} barcodes={barcodes} userName={userName} />
      ).toBlob();
        ...
        <div className="space-y-6">
          {filteredOrders.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-200 shadow-sm">
              ...
              <Link
                href="/events"
                className="px-8 py-3 bg-primary hover:bg-[#d90d6b] text-white font-bold rounded-full inline-flex items-center gap-2"
              >
