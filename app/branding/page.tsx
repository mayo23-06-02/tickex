...
function BrandingPageContent() {
  ...
  const handleUpdateComponents = useCallback(
    (components: any[]) => {
      setPages((prevPages) =>
        prevPages.map((p) =>
          p.id === currentPageId ? { ...p, components } : p
        )
      );
    },
    [currentPageId]
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-[#0f172a]">
            Select Event to Design
          </h1>
          <p className="text-[#64748b]">
            Choose an event to build a microsite for.
          </p>
        </div>
        {loading ? (
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-[#64748b]">
              <span className="font-mono">
                https://tickex.co.sz/sites/{currentEvent?.slug || eventId}
                {currentPage?.slug !== "/" ? currentPage?.slug : ""}
              </span>
            </p>
          </div>
          <a
            href={`/sites/${currentEvent?.slug || eventId}${
              currentPage?.slug !== "/" ? currentPage?.slug : ""
            }`}
            target="_blank"
