...
import { DashboardTimelineNotifications } from "@/components/dashboard/DashboardTimelineNotifications";
...
export default async function Dashboard({
  ...
  const currentEventId =
    typeof rawEventParam === "string" && rawEventParam.length > 0
      ? rawEventParam
      : "all";
    ...
    try {
      ...
      if (eventIds.length > 0) {
        ...
        if (recentOrders.length > 0) {
          ...
          lastSaleMinutes = Math.max(
            0,
            Math.floor(
              (Date.now() - new Date(lastSale).getTime()) / (60 * 1000)
            )
          );
          const ordersInHour = (
          ...
          ).filter((o) => new Date(o.createdAt) > oneHourAgo);
          const ticketsInHour = ordersInHour.reduce(
            (sum, o) =>
              sum +
        ...
        timelineItems = milestones.map((m) => ({
          ...
          subtitle: m.dueDate
            ? `Due ${new Date(m.dueDate as Date).toLocaleDateString()}`
            : "",
        }));
      }
    } catch (e) {
      ...
      <div className="mb-8 flex justify-between items-start">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            className="gap-2"
            href={`/event-timeline${
              currentEventId !== "all" ? `?eventId=${currentEventId}` : ""
            }`}
          >
      ...
      {metrics.activeEvents === 0 ? (
        ...
        <>
          ...
          <div className="space-y-6">
            <DashboardTimelineNotifications eventId={currentEventId} />
...
async function computeAnalytics(userId: string, currentEventId: string) {
  ...
  if (eventIds.length > 0) {
    ...
    const ticketTypes = await TicketType.find({ event: { $in: eventIds } })
      .select("_id name quantitySold quantityTotal event")
      .lean();
    const idToName = new Map<string, string>(
      ticketTypes.map((tt) => [tt._id.toString(), tt.name])
    );
    ...
    for (const o of paid as OrderLite[]) {
      ...
      eventRevenue.set(
        evId,
        (eventRevenue.get(evId) || 0) + (o.totalAmount || 0)
      );
      if (!perEventChildren.has(evId))
        perEventChildren.set(evId, new Map<string, number>());
      const children = perEventChildren.get(evId)!;
      for (const it of o.items || ([] as OrderItemLite[])) {
        const tid = String(
          (it as unknown as { ticketTypeId: unknown }).ticketTypeId
        );
    ...
    treemap = Array.from(eventRevenue.entries()).map(([evId, value]) => ({
      ...
      value,
      children: Array.from(
        (perEventChildren.get(evId) || new Map()).entries()
      ).map(([name, v]) => ({ name, value: v })),
    }));
    const totalSold = ticketTypes.reduce(
      (s, tt) => s + (tt.quantitySold || 0),
      0
    );
    const totalCap = ticketTypes.reduce(
      (s, tt) => s + (tt.quantityTotal || 0),
      0
    );
    ...
    <div className="space-y-6">
      <div className="bg-card rounded-xl p-6 shadow-sm border border-border">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {data.capacity.sold} of {data.capacity.total} tickets sold
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          <div className="p-4 border border-border rounded-lg">
            <div className="text-xs font-semibold text-muted-foreground mb-2">
              Stacked Sales Over Time
            </div>
            <div className="h-40 flex items-end gap-2">
              {data.stacked.map((d, i) => {
                const total = stackedKeys.reduce(
                  (s, k) => s + (d.series[k] || 0),
                  0
                );
                const height =
                  total > 0
                    ? Math.min(100, Math.round((total / (total || 1)) * 100))
                    : 4;
                return (
                  <div
                    key={i}
